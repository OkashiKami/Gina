...
public class PlayerData : IDisposable
{
    ...
    public ItemData[] actionbar = new ItemData[12];
    ...
    [JsonIgnore] public PlayerData Copy => new PlayerData(this);
    ...
    public delegate void OnActionbarChanged(ItemData[] actionbar); public event OnActionbarChanged onActionbarChanged;
    public delegate void OnCharacterChanged(ItemData[] character); public event OnCharacterChanged onCharacterChanged;

    public PlayerData() { }
    public PlayerData(PlayerData value = null)
    {
        if (value == null)
            return;
        ...
        Array.Copy(value.actionbar, this.actionbar, 12);
        Array.Copy(value.character, this.character, 15);

        this.file = value.file;
    }
    public void Set (PlayerData value = null)
    {
        if (value == null)
            return;
        this.Name = value.Name;
        ...
        onActionbarChanged?.Invoke(this.actionbar);
    ...
    public void ModifyActionbar(int index = -1, ItemData value = null)
    {
        if (index >= 0) actionbar[index] = value;
        else
        {
            bool alreadyAdded = false;
            // Check every slot to see if item is already there then add onto it if the item is stakable
            ...
            if (!alreadyAdded)
            {
                ...
                for (int i = 0; i < actionbar.Length; i++)
                {
                    if (actionbar[i] == null || !actionbar[i].IsValid)
                    {
                        actionbar[i] = value;
                        break;
                    }

                    var slot = actionbar[i];
                    var item = value;
                    if (item.id == slot.id)
                    {
                        if (slot.isStackable)
